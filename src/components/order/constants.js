// Группы опций
export const OPTION_GROUP_EDIT = "edit";
export const OPTION_GROUP_INTEGRATIONS = "integrations";
// Опции
export const OPTION_SOURCE_FROM_URL = "from_url";
export const OPTION_SOURCE_FROM_SHOP = "from_shop";
export const OPTION_SOURCE_FROM_ARCHIVE = "from_archive";
export const OPTION_EDIT_EDIT_CONTACTS = "edit_contacts";
export const OPTION_EDIT_ADD_CLIENT_COUNTERS = "add_client_counters";
export const OPTION_EDIT_CLIENT_CHANGES = "client_changes";
export const OPTION_INTEGRATIONS_COLLECT_LEADS = "collect_leads";
export const OPTION_INTEGRATIONS_SEND_LEADS_TO_PP = "send_leads_to_pp";
export const OPTION_PLACEMENT_DOWNLOAD_LANDING = "download_landing";
export const OPTION_PLACEMENT_DEPLOY_TO_CLIENT_SERVER = "deploy_to_client_server";
